...
def require_employer(user: dict):
    ...
    if user.get("role", "").lower() != "employer":
        raise HTTPException(
            status_code=403, detail="Only employers can access this resource"
        )
...
async def get_company_profile(user: dict):
    ...
    profile = await db.company_profiles.find_one({"employerEmail": user.get("email")})
...
async def save_company_profile(payload: CompanyProfilePayload, user: dict):
    ...
    existing = await db.company_profiles.find_one({"employerEmail": user.get("email")})

    if existing:
        doc["createdAt"] = existing.get("createdAt", datetime.utcnow())
        await db.company_profiles.update_one({"_id": existing["_id"]}, {"$set": doc})
...
async def create_employer_job(payload: dict, user: dict):
    ...
    job_doc = {
        ...
        "applyLink": str(payload.get("applyLink", "")).strip(),
    ...
    if not job_doc["company"]:
        ...
        job_doc["company"] = (
            company_profile.get("companyName", "") if company_profile else ""
        )
...
async def get_employer_jobs(user: dict):
    ...
    jobs_cursor = db.jobs.find({"employerEmail": user.get("email")}).sort(
        "createdAt", -1
    )
    jobs = [serialize_doc(job) async for job in jobs_cursor]
...
async def update_employer_job(job_id: str, payload: dict, user: dict):
    ...
    update_doc = {field: payload[field] for field in allowed_fields if field in payload}

    if "skills" in update_doc and isinstance(update_doc["skills"], str):
        update_doc["skills"] = [
            skill.strip() for skill in update_doc["skills"].split(",") if skill.strip()
        ]
...
async def get_employer_dashboard(user: dict):
    ...
    total_jobs = await db.jobs.count_documents({"employerEmail": employer_email})
    active_jobs = await db.jobs.count_documents(
        {"employerEmail": employer_email, "status": "active"}
    )
    ...
    shortlisted = await db.applications.count_documents(
        {"employerEmail": employer_email, "status": "shortlisted"}
    )

    profile = await db.company_profiles.find_one({"employerEmail": employer_email})
    ...
    recent_jobs_cursor = (
        db.jobs.find({"employerEmail": employer_email}).sort("createdAt", -1).limit(5)
    )
    recent_jobs = [serialize_doc(job) async for job in recent_jobs_cursor]
...
async def get_employer_applicants(user: dict):
    ...
    applications_cursor = db.applications.find({"employerEmail": employer_email}).sort(
        "createdAt", -1
    )
    applications = [app async for app in applications_cursor]
    ...
    for app in applications:
        ...
        if not job:
            job = {
                "title": app.get("jobTitle", ""),
                "skills": app.get("jobSkills", []),
                "description": app.get("jobDescription", ""),
                "experience": app.get("jobExperience", ""),
            }
        ...
        enriched = {
            **app,
            "jobTitle": job.get("title") or app.get("jobTitle", "Unknown Job"),
        ...
        enriched_applicants.append(serialize_doc(enriched))

    enriched_applicants.sort(
        key=lambda item: int(float(item.get("matchScore", 0))), reverse=True
    )
    ...
    pending = len(
        [
            app
            for app in enriched_applicants
            if str(app.get("status", "pending")).lower()
            in ["pending", "under review", "review"]
        ]
    )
    strong_matches = len(
        [
            app
            for app in enriched_applicants
            if int(float(app.get("matchScore", 0))) >= 80
        ]
    )
...
async def update_applicant_status(application_id: str, payload: dict, user: dict):
    ...
    allowed = ["pending", "under review", "shortlisted", "rejected", "interview"]
    if status not in allowed:
        raise HTTPException(status_code=400, detail="Invalid applicant status")

    result = await db.applications.update_one(
        {"_id": ObjectId(application_id), "employerEmail": user.get("email")},
...
async def rank_candidates_for_job(job_id: str, user: dict):
    ...
    job = await db.jobs.find_one(
        {"_id": ObjectId(job_id), "employerEmail": user.get("email")}
    )
    ...
    for app in applications:
        ...
        ranked.append(serialize_doc(app))

    ranked.sort(key=lambda item: int(float(item.get("matchScore", 0))), reverse=True)
